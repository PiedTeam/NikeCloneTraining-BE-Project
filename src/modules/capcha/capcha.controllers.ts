...
export const verifyTokenCapchaController = async (req: Request, res: Response, next: NextFunction) => {
  const { capcha } = req.body

  if (!capcha) {
    return res.status(StatusCodes.BAD_REQUEST).json({ message: CAPCHA_MESSAGES.TOKEN_REQUIRED })
  }
  ...
  request(verifyUrl, (err, response, body) => {
    if (err) {
      return res.status(StatusCodes.BAD_REQUEST).json({ message: CAPCHA_MESSAGES.WRONG_TOKEN })
    }
